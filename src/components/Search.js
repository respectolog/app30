...
export function Search(props) {
  ...
  for (let item of categories) {
    if (item.name.toLowerCase().indexOf(s_word.toLowerCase()) !==-1 && s_word !== ''){
      s_mass.push({category: item.name, id: item.id});
    }
    if (item.categories) {
      for (let sub of item.categories) {
        if (sub.title_ru.toLowerCase().indexOf(s_word.toLowerCase())!==-1 && s_word !== ''){
          s_mass.push({category: sub.title_ru, id: sub.id});
        }
        if(sub.subcategories){
          for (let subsub of sub.subcategories) {
            if (subsub.title_ru.toLowerCase().indexOf(s_word.toLowerCase())!==-1 && s_word !== ''){
